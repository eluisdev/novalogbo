...
class InvoiceController extends Controller
{
    ...
    // {
    //     $request->validate([
    //         'id' => 'required|exists:billingnotes,id',
    ...
    //             $taxRate = 0.00; // IVA de Bolivia (13%)
    ...
    {
        $request->validate([
            'id' => 'required|exists:billing_notes,id',
            'visible' => 'required|boolean',
            'is_parallel' => 'sometimes|boolean'
        ]);
        $billingNoteId = $request->id;
        $visible = $request->visible ?? true;
        $isParallel = $request->is_parallel ?? false;
        $billingNote = BillingNote::with(['customer', 'items'])->find($billingNoteId);
        DB::beginTransaction();
        try {
            dd(Invoice::where('billing_note_id', $billingNoteId)
                ->where('is_parallel', $isParallel)
                ->first());
            $existingInvoice = Invoice::where('billing_note_id', $billingNoteId)
                ->where('is_parallel', $isParallel)
                ->first();

            if (!$existingInvoice) {
                $rateValue = $billingNote->exchange_rate;
                $subtotal = 0;
                $taxRate = 0.00; // Tax set to 0%

                foreach ($billingNote->items as $item) {
                    if ($isParallel) {
                        // Solo considerar items con diferencia en tipo de cambio
                        if ($item->exchange_rate != $billingNote->exchange_rate) {
                            $amount = $item->is_amount_parallel ? $item->amount_parallel : $item->amount;
                            $subtotal += $amount;
                        }
                    } else {
                        // Considerar todos los items
                        $amount = $item->is_amount_parallel ? $item->amount_parallel : $item->amount;
                        $subtotal += $amount;
                    }
                }
                ...
                $invoice = new Invoice([
                    ...
                    'exchange_rate' => $rateValue,
                    'status' => 'issued',
                    'notes' => "Factura generada automáticamente de nota de cobranza #{$billingNote->note_number}",
                    'user_id' => Auth::id(),
                    'customer_nit' => $billingNote->customer->NIT,
                ...
                foreach ($billingNote->items as $item) {
                    if ($isParallel && $item->exchange_rate == $billingNote->exchange_rate) {
                        continue; // Saltar items sin diferencia en tipo de cambio
                    }

                    $itemAmount = $item->is_amount_parallel ? $item->amount_parallel : $item->amount;
                    $itemTaxAmount = $itemAmount * $taxRate;
                    $itemTotal = $itemAmount + $itemTaxAmount;

                    $invoiceItem = new InvoiceItem([
                        'invoice_id' => $invoice->id,
                        'description' => $isParallel
                            ? "Ajuste por diferencia de cambio - " . $item->description
                            : $item->description,
                        'quantity' => 1,
            ...
            DB::commit();
            return $this->generateWordDocument($invoice, $visible, $isParallel);
        } catch (\Exception $e) {
            DB::rollBack();
            return back()->withErrors(['error' => 'Error al crear la factura: ' . $e->getMessage()]);
        }
    ...
    private function generateWordDocument($invoice, $visible, $isParallel = false)
    {
        $invoice->load(['customer', 'items', 'billingNote']);
        ...
        $phpWord->getSettings()->setThemeFontLang(new Language(Language::ES_ES));
        $properties = $phpWord->getDocInfo();
        $properties->setTitle($isParallel ? 'Factura Ajuste Tipo Cambio' : 'Factura');
        $properties->setCreator('NOVALOGISTIC BOLIVIA SRL');
        $properties->setCompany('NOVALOGISTIC BOLIVIA SRL');
        $phpWord->setDefaultFontName('Calibri');
        $phpWord->setDefaultFontSize(9);
        ...
        $bannerTable->addRow(600);
        $bannerCell = $bannerTable->addCell(10000, ['bgColor' => 'E8EEF4', 'valign' => 'center', 'borderSize' => 1, 'borderColor' => '1F497D']);
        $bannerCell->addText($isParallel ? 'FACTURA AJUSTE TIPO CAMBIO' : 'FACTURA', $titleStyle, $centerOptions);
        $bannerCell->addText('N°: ' . $invoice->invoice_number, ['bold' => true, 'size' => 13, 'color' => '1F497D'], $centerOptions);
        $section->addTextBreak(1);
        ...
        if ($invoice->billingNote) {
            ...
            $detailsTable->addCell(3000)->addText($invoice->billingNote->note_number);
        }

        if ($isParallel) {
            $detailsTable->addRow();
            $detailsTable->addCell(2000)->addText('Tipo de factura:', ['bold' => true]);
            $detailsTable->addCell(3000)->addText('AJUSTE POR DIFERENCIA DE CAMBIO');
        }
        ...
        foreach ($invoice->items as $item) {
            ...
            if ($isParallel && isset($item->original_exchange_rate)) {
                $description .= " (TC Original: " . number_format($item->original_exchange_rate, 2) . ")";
            }

            $conceptsTable->addCell(4000)->addText($description, null, $paragraphOptions);
            $conceptsTable->addCell(1000)->addText($item->quantity, null, $centerOptions);
            $conceptsTable->addCell(1500)->addText($item->currency . ' ' . number_format($item->unit_price, 2), null, $rightOptions);
            $conceptsTable->addCell(1500)->addText($item->currency . ' ' . number_format($item->total, 2), null, $rightOptions);
            $conceptsTable->addCell(1500)->addText('BS ' . number_format($item->total * $invoice->exchange_rate, 2), null, $rightOptions);
        ...
        // Generar el archivo
        $filename = $isParallel
            ? 'factura-ajuste-' . $invoice->invoice_number . '.docx'
            : 'factura-' . $invoice->invoice_number . '.docx';
        $tempFile = tempnam(sys_get_temp_dir(), 'PHPWord');
        $writer = IOFactory::createWriter($phpWord, 'Word2007');
        $writer->save($tempFile);
