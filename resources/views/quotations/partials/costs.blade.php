...
<div class="p-6 border-b-2 border-blue-600 bg-white shadow-sm">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        @foreach ($costs as $cost)
            ...
            <div
                class="cost-card bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition-all duration-200 shadow-sm hover:shadow-md overflow-hidden {{ $isEnabled ? 'ring-1 ring-blue-200' : '' }}">
                <div class="p-4">
                    <div class="flex items-center mb-3">
                        <input type="checkbox" id="cost_{{ $cost->id }}" name="costs[{{ $cost->id }}][enabled]"
                            value="1"
                            class="cost-toggle h-4 w-4 focus:ring-blue-500 text-blue-600 border-gray-300 rounded"
                            data-cost-id="{{ $cost->id }}" onchange="toggleCostFields({{ $cost->id }})"
                            @checked($isEnabled)>
                        <label for="cost_{{ $cost->id }}"
                            class="ml-2 block text-sm font-medium text-gray-700 cursor-pointer">
                            {{ $cost->name }}
                        </label>
                    </div>
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="number" step="0.01" min="0" name="costs[{{ $cost->id }}][amount]"
                            ...
                            @disabled($isDisabled) data-cost-id="{{ $cost->id }}" placeholder="0.00">
